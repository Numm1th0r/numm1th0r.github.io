...
(function () {
    ...
    const CACHE_KEY_STATUSES    = 'evok_statuses_v1';
    ...
    // Akkor számít késésnek, ha van "kései/kési/késett/késve/késő" sor,
    // ÉS NINCS "újra leadott munka" sor (utóbbi azt jelenti, eredetileg
    // időben adta le, csak újra leadta — ilyenkor a "késett" jelzés
    // megtévesztő, ezért nem számoljuk be).
    // Class alapján és szöveg alapján is detektálunk (régi cache-elt
    // adatokon is jól működik).
    function isLateSubmission(data) {
        if (!data.lines || !data.lines.length) return false;
        const hasLate = data.lines.some(l =>
            (l.cls && l.cls.includes('late')) ||
            /kés(ei|i|ett|ve|ő)/i.test(l.text || '')
        );
        const hasResubmit = data.lines.some(l =>
            (l.cls && l.cls.includes('reminder')) ||
            /újra\s*leadott/i.test(l.text || '')
        );
        return hasLate && !hasResubmit;
    }
    async function cellaAllapotok(itemMap, mode) {
        ...
        for (const itemId in itemMap) {
            ...
            userIds.forEach(uid => {
                ...
                if (isLateSubmission(data)) td.dataset.evokLate = '1';
    ...
    function statuszSzamlalo() {
        document.querySelectorAll('tr.userrow:not(.evok-hidden)').forEach(tr => {
            ...
            tr.querySelectorAll('td.gradecell[data-itemid]:not(.evok-future-col)').forEach(td => {
                ...
                if (td.dataset.evokLate === '1') lateCount++;
            ...
            if (lateCount > 0) {
                ...
                lateDiv.className = 'evok-counter-line evok-late';
                lateDiv.textContent = `⏰ Késett: ${lateCount}`;
                wrap.appendChild(lateDiv);
            }
